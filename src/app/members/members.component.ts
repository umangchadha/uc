...
export class MembersComponent  {
  somerowselected: boolean =false;
  displayedColumns = ['select','ID', 'name', 'email','gender','ip'];
  dataSource = new MatTableDataSource(ELEMENT_DATA);
  selection = new SelectionModel<Element>(true, []);

  constructor(private userService: UserService,private pass:LinkDataService) {
  }
 ...
 // get users from secure api end point
  ...
  checkClicked(){
  if(this.selection.selected.length>0) this.somerowselected=true;
else{
...
const ELEMENT_DATA: Element[] = [{"id":1,"first_name":"Wanda","last_name":"Morales","email":"wmorales0@e-recht24.de","gender":"Female","ip_address":"168.21.40.204"},
